...
{ 
   ...
   //  SD_SendCmd(16,Bytes,0xff); 
    ...
    retry = 0;
	 ...
	 ma=addr%512;	  //计算多余的字节
	 mb=addr-ma;	  //找到字节地址 的扇区首
	 

    do
    {
    // 写入CMD17 	读取命令
    temp=SD_SendCmd(CMD17,mb,0x01);
    retry++;
    }
		
	while((temp!=0x00) && (retry < TRY_TIME)); 	  //等待命令成功

	
    if (retry >= TRY_TIME) 		  //如果不成功返回
    {
	...
	for(i=0;i<ma;i++)		  //循环掉多余的字节数
	SPI_SendByte(0xff); 

    for(i=0;i<Bytes;i++) 	  //读取要读取的字节到buf中       
    buf[i] = SPI_SendByte(0xff);  //连续读出指定 长度字节数据到buf
 
	for(i=0;i<(512-ma);i++)	   //循环掉剩余的字节数
	SPI_SendByte(0xff); 
