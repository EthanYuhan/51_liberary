...
extern FATFS fatfs;
u32   gbk12_sector;		   //GBK码首扇区


//提取 TF卡中  GBK点阵码  首扇区  为直接以TF为字库读取即显示做准备 
//读取 库开始扇区
...
FRESULT res;
  ...
  if(res == FR_OK)				   //寻找成功  读取文件的开始簇 并计算其扇区
    gbk12_sector=(fatfs.org_clust-fatfs.dirbase)*fatfs.csize+fatfs.database;
	              //gbk12_sector=（当前文件簇-目录所在簇）*每簇扇区数+ 根目录扇区数 
				  //这里的计算在 FAT32学习笔记 elh.pdf 有介绍
...
u8 PutGB1616(u16 x, u16  y, u8 Disp_char[2], u16 fColor,u16 bColor,u8 mode)
{
   ...
   if(bColor!=0x0001)							//支持背景颜色设置
    {														 //注意 不管设置哪一显示方向 前提是扫描要对应
	    if(mode==0)LCD_SetWindows(x,y,  x+16-1, y+16-1);	 // 设置为纵向显示
		if(mode==1)LCD_SetWindows(x,y,  x+16-1, y+16-1);// 设置为纵向显示
	    //显示出来 
	    for(i=0;i<32;i++)
	    {
   ...
   else										//不支持背景颜色设置  只显示字体颜色 背景颜色与刷屏时一致
	{
		...
		for(i=0;i<32;i++)
		{
	        for(j=0;j<8;j++) 
	        {
			    LCD_SetWindows(x,y,x,y);	//显示坐标要一个点一个点显示
	            if(tmp_Char_Model[i] & 0x80) 
	                   {
	                	    LCD_WR_DATA(fColor);		//显示汉字的颜色
							x++;						//x自加
...
//新建从tf字库  屏幕打印16*16汉字
void GUI_DrawFont16_tf(u16 x, u16 y, u16 fc, u16 bc, u8 *s,u8 mode)
{
	u8 i,j;
	u16 k;
	u16 HZnum;
...
{					
    ...
    { 
        ...
        {   
			...
			//GUI_DrawFont16(x,y,fc,bc,str,mode); //直接先在这个地方
												   //修改，修改为tf卡字库显示
			
			PutGB1616(x, y, str, fc,bc,0); //tf卡字库显示
...
{  
	if (bgcolor == 1) bgcolor = BLACK;
	
	Show_Str(x1, y1, dcolor, bgcolor, str,16,0);
