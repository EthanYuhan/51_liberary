...
/***************----- W5500 GPIO定义 -----***************/
sbit W5500_RST = P3^4;	//定义W5500的RST引脚
//sbit W5500_INT = P3^3;	//定义W5500的INT引脚 中断未开 采用扫描寄存器方式 
sbit W5500_SCS = P1^2;	//定义W5500的CS引脚	 
	
//SPI	   STC15 SPI对应IO如下     硬件SPI
//W5500_SCLK 	W5500的SCLK引脚		   P43
//W5500_MISO 	W5500的MISO引脚 	   P41
//W5500_MOSI 	W5500的MOSI引脚	 	   P40
...
//端口数据缓冲区 
u8 Rx_Buffer[40];	//端口接收数据缓冲区 
u8 Tx_Buffer[40];	//端口发送数据缓冲区 
...
{
    return SPI_SendByte(dat);	//写1个字节数据
}
...
{
	SPI_SendByte(dat/256);//写数据高位   相当于dat>>8
...
u8 Read_W5500_1Byte(u16 reg)
{
	...
	i=SPI1_Send_Byte(0x00);//发送空数据 得到返回数据给i
...
u16 Read_W5500_SOCK_2Byte(SOCKET s, u16 reg)
{
	...
	i=SPI1_Send_Byte(0x00);//发送空数据 	读取高字节数据
	i=i<<8;				   //偏移高字节数据	
	i+=SPI1_Send_Byte(0x00);//发送空数据    读取低字节数据
...
u16 Read_SOCK_Data_Buffer(SOCKET s, u8 *dat_ptr)
{
	...
	else//如果最大地址超过W5500接收缓冲区寄存器的最大地址
	{
		...
		{
			j=SPI1_Send_Byte(0x00);//发送空数据	  读取1个字节数据给j
			*dat_ptr=j;//将读取到的数据保存到数据保存缓冲区
			dat_ptr++;//数据保存缓冲区指针地址自增1
		}
		W5500_SCS=1; //拉高CS  取消片选
...
void W5500_Hardware_Reset(void)
{
	...
	while((Read_W5500_1Byte(PHYCFGR)&LINK)==0) //死循环等待以太网连接完成	这里要将网线连接到路由和模块上 否则将无法通过
	{
	  ...
	  if(xun==40000)
	  {
	    UartSendStr("以太网连接失败，可能未连接网线！\r\n");
		xun=0;
	  }
	}
	UartSendStr("以太网连接成功，准备进入通讯状态！\r\n");
	delay1ms(2000);
