...
//  Lcd_WR_Reg(0x0022);			 //发送设置
   ...
   LCD_Scan_Dir(4);    //方向先不改
  
  Show_Str(240,0,BLUE,YELLOW,"ov7670    ",16,0); 	
  Show_Str(240,16,BLUE,YELLOW,"摄像测试版",16,0); 
  Show_Str(240,224,BLUE,YELLOW,"      返回",16,0); 
  
  
  
  if( Ov7670_init())	 //初始化 ov7670  
  {															 //初始化不成功
   Show_Str(60,150,RED,YELLOW,"OV7670 Init Error!",16,0); 
   delay1ms(2000);
   EX1=0;			   //关闭外部中断0
   EA=0;			   //关闭总中断
   sign=1;
...
//  Lcd_WR_Reg(0x0022);			 //发送设置
  ...
  OV7670_Window_Set(10,176,240,240);	//设置窗口
 ...
 while(1)
 {
  ...
  LCD_SetWindows(0,0,239,239);	  //设置显示范围  显示为240*240 
 ...
 void zhen() interrupt 2 //外部中断1  P3.2  判断帧数据  --> P33
 {
